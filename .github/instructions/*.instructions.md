...
- Flag improper use of `is` vs `==` for value comparisons (e.g., `if x is "string"` is undefined behavior in CPython for non-interned strings).
...
- Flag `.catch()` or `try/catch` missing on all `async`/`await` call sites in non-trivial code paths.
...
### Error Handling
- Flag errors that are caught and re-thrown without adding context — this loses the stack trace without benefit.
